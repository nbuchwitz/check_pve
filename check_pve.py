...
class CheckPVE:
    ...
    def _get_url(self, command):
        return self.API_URL.format(
            hostname=self._api["host"], command=command, port=self._api["port"]
        )
    ...
    def _request(self, url, method="get", data=None, params=None):
        ...
        try:
            if method == "post":
                ...
                output(
                    CheckState.CRITICAL, "Unsupport request method: {}".format(method)
                )
        except requests.exceptions.ConnectTimeout:
            output(
                CheckState.UNKNOWN, "Could not connect to PVE API: Connection timeout"
            )
    ...
    def check_api_value(self, url: str, message: str, **kwargs):
        ...
        if isinstance(result, dict):
            ...
            self._add_perfdata(
                "used", used, default_threshold=False, max=total, unit=self._unit
            )
        else:
        ...
        for virtual_machine in data:
            if idx in [virtual_machine["name"], virtual_machine["vmid"]]:
                ...
                if virtual_machine["status"] != expected_state:
                    ...
                    if node and node != virtual_machine["node"]:
                        self.check_message = "{} '{}' is {}".format(
                            vm_type, virtual_machine["name"], expected_state
                        )
                        self.check_message += (
                            ", but located on node '{}' instead of '{}'".format(
                ...
                if virtual_machine["status"] == "running" and not only_state:
                    ...
                    if self._options.get("values_bytes", False):
                        memory = self._scale_value(virtual_machine["mem"], self._unit)
                        self._add_perfdata(
                            "memory",
                            memory,
                            unit=self._unit,
                            default_threshold=False,
                            max=self._scale_value(
                                virtual_machine["maxmem"], self._unit
                            ),
                        )
    ...
    def _check_disks(self, node: str, ignore_disks: list = None):
        ...
        self._check_thresholds(
            values, self.check_message, default_threshold=list(values.keys())
        )
    ...
    def _check_replication(self, node: str, vmid: int = None):
        ...
        if len(failed_jobs) > 0:
            ...
            self.check_message = "No failed replication jobs on {}".format(node)
    ...
    def _check_services(self, node: str):
        ...
        if failed:
            ...
            message += "\n".join(
                [
                    "- {} ({}) is not running".format(name, desc)
    ...
    def _check_subscription(self, node: str):
        ...
        if data["status"] == "Inactive":
            ...
            message = "{} is valid until {}".format(
                subscription_product_name, subscription_due_date
            )
            message_warning_critical = "{} will expire in {} days ({})".format(
                subscription_product_name, delta, subscription_due_date
            )
    ...
    def _check_cluster_status(self):
        ...
        if quorate:
            ...
            if nodes_count > nodes_online_count:
                self.check_result = CheckState.WARNING
                self.check_message = (
                    "Cluster '{}' is healthy, but {} node(s) offline:\n'".format(
                ...
                self.check_message = "Cluster '{}' is healthy'".format(cluster_name)
    ...
    def _check_zfs_fragmentation(self, node: str, name: str = None):
        ...
        for pool in data:
            ...
            if (name is not None and name == pool["name"]) or name is None:
                self._zfs_fragmentation_process_pool(name, pool, warnings, critical)

        if not found:
            self.check_result = CheckState.UNKNOWN
            self.check_message = (
                "Could not fetch fragmentation of ZFS pool '{}'".format(name)
            )
            return
        ...
        if warnings or critical:
            ...
            if name is not None:
                self.check_message = (
                    "Fragmentation of ZFS pool '{}' is above thresholds: {} %".format(
                        name, value
                    )
                )
            else:
                message = (
                    "{} of {} ZFS pools are above fragmentation thresholds:\n\n".format(
    ...
    def _check_zfs_health(self, node: str, name=None):
        ...
        if not found:
            ...
            if unhealthy:
                ...
                message += "\n".join(
                    [
                        "- {} ({}) is not healthy".format(pool["name"], pool["health"])
                        for pool in unhealthy
                    ]
                )
                self.check_message = message
            else:
    ...
    def _check_version(self, min_version=None):
        ...
        if not data["version"]:
            ...
            self.check_message = "Current pve version '{}' ({}) is lower than the min. required version '{}'".format(
                data["version"], data["repoid"], min_version
            )
        else:
            self.check_message = (
                "Your pve instance version '{}' ({}) is up to date".format(
                    data["version"], data["repoid"]
                )
            )
    ...
    def _check_thresholds(self, values, message, **kwargs):
        ...
        for metric, value in values.items():
            ...
            value_critical = self._threshold_critical(
                metric, metric in default_threshold
            )
    ...
    def _add_perfdata(self, name, value, **kwargs):
        ...
        perfdata += ";"
        if (
            not ignore_thresholds or default_threshold
        ) and threshold_critical is not None:
    ...
    def check(self, mode: str, options: dict = {}):
        ...
        checks = {
            ...
            "disk-health": lambda: self._check_disks(
                options["node"], options.get("ignore_disks", [])
            ),
            ...
            "replication": lambda: self._check_replication(
                options["node"], options.get("vmid", None)
            ),
            "ceph-health": self._check_ceph_health,
            "zfs-health": lambda: self._check_zfs_health(
                options["node"], options["name"]
            ),
            "zfs-fragmentation": lambda: self._check_zfs_fragmentation(
                options["node"], options["name"]
            ),
        }
    ...
    def authentificate_password(self, username: str, password: str):
        ...
        self._api["cookies"]["PVEAuthCookie"] = self.get_ticket(username, password)
        ...
        for token in arg.split(","):
            ...
            if token_match:
                ...
                raise argparse.ArgumentTypeError(
                    "invalid threshold format: {}".format(token)
                ) from value_error
...
def compare_thresholds(
    threshold_warning: dict, threshold_critical: dict, comparator_func
):
...
def parse_args(args):
    """parse CLI args"""
    parser = argparse.ArgumentParser(description="Check command for PVE hosts via API")

    api_opts = parser.add_argument_group("API Options")

    api_opts.add_argument(
        "-e", "--api-endpoint", required=True, help="PVE api endpoint hostname"
    )
    ...
    secret_group.add_argument(
        "-p", "--password", dest="api_password", help="PVE API user password"
    )
    ...
    check_opts.add_argument(
        "--unit",
        choices=CheckPVE.UNIT_SCALE.keys(),
        default="MiB",
    ...
    check_opts.add_argument(
        ...
        dest="node",
        required=opts.mode
        not in ["cluster", "vm", "vm_status", "version", "ceph-health"],
...
if __name__ == "__main__":
    ...
    if cli_args.threshold_warning and cli_args.threshold_critical:
        if cli_args.mode != "subscription" and not compare_thresholds(
            cli_args.threshold_warning, cli_args.threshold_critical, lambda w, c: w <= c
        ):
            output(
                CheckState.UNKNOWN,
